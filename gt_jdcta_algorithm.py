...
def task_utility(ti, task_dict,_assigned_task,task_pre_assign, worker_dict, group, v, w_list, w_skill, alpha, worker_v, max_s,max_p,TC,FT):
    related_tasks= copy.deepcopy(FT[ti][:1])
    # print(ti)
    # result = w_skill // 10
...
def greedy_strategy(task_dict, worker_dict, workers, tasks,
    ...
    for i in disrupted_tasks:
        ...
        for r in skill_group[i].keys():
            ...
            if len(skill_list) > 0:
                # Greedy selection: choose the worker that minimizes the distance to the task
                best_worker = min(skill_list,
                                  key=lambda w: Euclidean_fun(task_dict[i]['Lt'], worker_dict[w]['Lw']))
                worker_list.append(best_worker)
                task_pre_assign[i]['group'][r] = best_worker
                strategy[best_worker]=i
                assigned_workers.add(best_worker)
        task_pre_assign[i]['list'] = worker_list
...
def get_available_task(task_dict, arrived_tasks, best_assign, TC):
    """
    """
    free_task = []
    for i in arrived_tasks:
        ...
        if current_time<max_time+1:
            ...
            while flag != 0:
                ...
                equ_strategy, task_pre_assign = gt_imp(
                    task_dict, worker_dict, workers, tasks, current_time, worker_v,reachable_dis,move_cost,alpha,max_s,max_p,TC,FC)
